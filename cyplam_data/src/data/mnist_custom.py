...
def read_datasets(dirnames, offset=0):
    ...
    frames = [analysis.read_frames(d.frame).reshape(-1, 1024) / 1024. for d in data]
...
print "Accuracy: ", sess.run(accuracy, feed_dict={x: features, y_: targets})
predicted = sess.run(tf.argmax(y, 1), feed_dict={x: features})
